...
const Admin = () => {
  ...
  const handleHeroImage = (e) => {
    ...
    const file = files[0];
    if (file) {
      setNewHero((prev) => ({ ...prev, [name]: file }));
      setHeroImagePreviews((prev) => ({ ...prev, [name]: URL.createObjectURL(file) }));
    }
  };
  ...
  const addHero = async (e) => {
    ...
    if (newHero.mainImage) {
      formData.append("mainImage", newHero.mainImage);
    }else{
    ...
    setNewHero({ title: "", mainImage: null  });
    ...
    <div className="admin-container">
      <header className="admin-header">
        ...
        <nav className="admin-nav">
          ...
          <button 
            className={activeTab === 'hero' ? 'nav-btn active' : 'nav-btn'}
      ...
      {activeTab === 'hero' && (
        <section className="hero-section">
          ...
          <form onSubmit={addHero} className="hero-form">
            <div className="form-group">
            ...
            {/* Preview only for new upload */}
            {heroImagePreviews.mainImage && (
              <div className="preview-row">
                <div className="preview-container">
                  <img 
                    src={heroImagePreviews.mainImage} 
                    alt="Hero preview" 
                    className="preview"
                  />
                  <span>Preview</span>
            ...
            <button disabled={isCreating} type="submit" className="submit-btn">
              {isCreating ? "Adding..." : "Add Hero Image"}
            </button>
          </form>
          ...
          <div className="hero-grid">
            ...
            {heros.length === 0 ? (
              <div className="no-data">No Hero Image added yet.</div>
            ) : (
              heros.map((hero) => (
